...
#include <linux/pkt_cls.h>
 ...
 SEC("tcx/ingress")
 int alertRSTflood(struct __sk_buff *skb) {
     ...
     if ((void *)(tcp + 1) > data_end)
         ...
         //RST ATTACK -> Just RST flag activated
     if (tcp->rst && !tcp->ack && !tcp->fin && !tcp->urg && !tcp->psh && !tcp->syn) {
         ...
         if (new_count > MAX_RST_THRESHOLD) {
             __u64 flag = 1;
